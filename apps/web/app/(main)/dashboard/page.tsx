...
import { AlertCircle, ArrowRight, CheckCircle, Clock, FileAudio, Loader2, Mic, Upload } from "lucide-react";
import type { MeetingRecord } from "@brevoca/contracts";
import { authedFetch } from "@/lib/client/authed-fetch";
import { useAppSession } from "@/components/AppSessionProvider";

const statusConfig: Record<string, { label: string; color: string; icon: typeof FileAudio }> = {
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    authedFetch("/api/meetings")
      .then(async (res) => {
        if (!cancelled && res.ok) {
          const payload = (await res.json()) as { items: MeetingRecord[] };
