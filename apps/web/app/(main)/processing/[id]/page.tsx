    ...
    <div className="p-6 lg:p-8">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-[var(--text-secondary)]">OpenAI가 전사와 회의록 요약을 순차적으로 생성하는 중입니다.</p>
        </div>
        ...
        <div className="mb-8 p-6 rounded-[var(--radius-xl)] bg-[var(--bg-surface)] border border-[var(--line-soft)]">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-sm font-mono text-[var(--mint-500)]">{job.progress}%</span>
          </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-[var(--text-secondary)]">
                        {status === "complete" ? "완료" : status === "active" ? "진행 중..." : "대기 중"}
                      </div>
                    </div>
                  </div>
                  {index < steps.length - 1 && (
                    <div className="ml-6 my-2">
                      <div className={`w-0.5 h-6 ${status === "complete" ? "bg-[var(--mint-500)]" : "bg-[var(--graphite-800)]"}`} />
                    </div>
                  )}
                </div>
              );
        ...
        <div className="p-6 rounded-[var(--radius-xl)] bg-[var(--bg-surface)] border border-[var(--line-soft)]">
          <div className="flex items-center justify-between mb-4">
            ...
            {retrying && <span className="text-sm text-[var(--text-secondary)]">재처리 요청 중...</span>}
          </div>
          <div className="space-y-2 max-h-60 overflow-y-auto font-mono text-sm">
            {job.logs.map((log, index) => (
              <div key={`${log}-${index}`} className="flex items-start gap-2 text-[var(--text-secondary)]">
...
function getStepStatus(currentStep: StepId, step: StepId, jobStatus: JobRecord["status"]) {
